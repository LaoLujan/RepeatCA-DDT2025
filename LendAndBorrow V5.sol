...
function approve(address _spender, uint256 _value) public view returns (bool success){ // Aprove to the spender the value of ___ tokens, and this transaction is only for read, not to make changes
    ...
    emit Aproval(msg.sender, _spender, _value); // Emit the information to the wallets/apps
    return true;
}
...
//This transaction is between the Owner and the spender only
    ...
    require(_value <= _allowed[_from][msg.sender], "Not enough allowence");// This is specified in the allowence function

    _balances[_from] -= _value; // This takes the tokens from the owner
    _balances[_to] += _ value; //And deposit then in the recipient, which is the spender
...
function lendToken(uint256 _amount) public return (bool success){
    require(msg.sender == lender, "Only lender can lend"); //This is requires that ONLY the lender can use this function
    require (_balances[lender] >= _amount, "Not enough tokens to lend"); // verifies the balance of the lender

    _balances[lender] -= _amount; // The actual transfer. This deduct the tokens from the lender account
